...
class _IOS26NativeTabBarState extends State<IOS26NativeTabBar> {
  ...
  Widget build(BuildContext context) {
    if (!kIsWeb && Platform.isIOS) {
      ...
      final assetIcons = widget.destinations
          .map((e) => e.iosAssetIcon)
          .toList();
      final selectedAssetIcons = widget.destinations
          .map((e) => e.iosAssetSelectedIcon)
          .toList();
  ...
  Future<void> _syncPropsToNativeIfNeeded() async {
    ...
    final assetIcons = widget.destinations.map((e) => e.iosAssetIcon).toList();
    final selectedAssetIcons = widget.destinations
        .map((e) => e.iosAssetSelectedIcon)
        .toList();
    final searchFlags = widget.destinations.map((e) => e.isSearch).toList();
    ...
    final lastLabelsSig = (_lastLabels ?? const <String>[]).join('|');
    final labelsSig = labels.join('|');
    final lastSymbolsSig = (_lastSymbols ?? const <String>[]).join('|');
    final symbolsSig = symbols.join('|');
    final lastAssetIconsSig = (_lastAssetIcons ?? <String?>[])
        .map((e) => e ?? '')
        .join('|');
    final assetIconsSig = assetIcons.map((e) => e ?? '').join('|');
    final lastSelectedAssetIconsSig = (_lastSelectedAssetIcons ?? <String?>[])
        .map((e) => e ?? '')
        .join('|');
    final selectedAssetIconsSig = selectedAssetIcons
        .map((e) => e ?? '')
        .join('|');

    if (lastLabelsSig != labelsSig ||
        lastSymbolsSig != symbolsSig ||
        lastAssetIconsSig != assetIconsSig ||
        lastSelectedAssetIconsSig != selectedAssetIconsSig) {
  ...
  void _cacheItems() {
    ...
    _lastAssetIcons = widget.destinations.map((e) => e.iosAssetIcon).toList();
    _lastSelectedAssetIcons = widget.destinations
        .map((e) => e.iosAssetSelectedIcon)
        .toList();
