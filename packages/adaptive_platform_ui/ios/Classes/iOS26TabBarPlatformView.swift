...
class iOS26TabBarPlatformView: NSObject, FlutterPlatformView, UITabBarDelegate {
    ...
    /// Resize tab icons to match the native tab bar expected size (~24pt).
    private static func resizedForTabBar(_ image: UIImage, maxSize: CGFloat = 24) -> UIImage {
        let size = image.size
        guard size.width > 0, size.height > 0 else { return image }
        let scaleFactor = min(maxSize / size.width, maxSize / size.height, 1.0)
    ...
    init(frame: CGRect, viewId: Int64, args: Any?, messenger: FlutterBinaryMessenger) {
        ...
        if let dict = args as? [String: Any] {
            ...
            if let raw = dict["assetIcons"] as? [Any] {
                assetIcons = raw.map { ($0 is NSNull) ? nil : ($0 as? String) }
            }
            if let raw = dict["selectedAssetIcons"] as? [Any] {
                selectedAssetIcons = raw.map { ($0 is NSNull) ? nil : ($0 as? String) }
            }
        ...
        func buildItems(_ range: Range<Int>) -> [UITabBarItem] {
            ...
            for i in range {
                ...
                if isSearch {
                    ...
                    // Prefer iOS asset icons (UIImage(named: ...)) if provided.
                    let assetName = (i < assetIcons.count) ? assetIcons[i] : nil
                    if let assetName = assetName, !assetName.isEmpty, let assetImage = UIImage(named: assetName) {
                        image = Self.resizedForTabBar(assetImage).withRenderingMode(.alwaysTemplate)
                        let selectedName = (i < selectedAssetIcons.count) ? selectedAssetIcons[i] : nil
                        if let selectedName = selectedName, !selectedName.isEmpty, let selectedAsset = UIImage(named: selectedName) {
                            selectedImage = Self.resizedForTabBar(selectedAsset).withRenderingMode(.alwaysTemplate)
                        } else {
                            selectedImage = image
                        }
    ...
    private func handleMethodCall(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "setItems":
            ...
            var assetIcons: [String?] = []
            var selectedAssetIcons: [String?] = []
            if let raw = args["assetIcons"] as? [Any] {
                assetIcons = raw.map { ($0 is NSNull) ? nil : ($0 as? String) }
            }
            if let raw = args["selectedAssetIcons"] as? [Any] {
                selectedAssetIcons = raw.map { ($0 is NSNull) ? nil : ($0 as? String) }
            }
    ...
    private func rebuildItemsWithCurrentColors() {
        ...
        for i in 0..<currentLabels.count {
            ...
            if isSearch {
                ...
                // Prefer iOS asset icons if available
                let assetName = (i < currentAssetIcons.count) ? currentAssetIcons[i] : nil
                if let assetName = assetName, !assetName.isEmpty, let assetImage = UIImage(named: assetName) {
                    image = Self.resizedForTabBar(assetImage).withRenderingMode(.alwaysTemplate)
                    let selectedName = (i < currentSelectedAssetIcons.count) ? currentSelectedAssetIcons[i] : nil
                    if let selectedName = selectedName, !selectedName.isEmpty, let selectedAsset = UIImage(named: selectedName) {
                        selectedImage = Self.resizedForTabBar(selectedAsset).withRenderingMode(.alwaysTemplate)
                    } else {
                        selectedImage = image
