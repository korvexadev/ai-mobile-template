...
class iOS26NativeTabBarManager: NSObject {
    ...
    private func resizedForTabBar(_ image: UIImage, maxSize: CGFloat = 24) -> UIImage {
        let size = image.size
    ...
    private func enableNativeTabBar(tabs: [TabConfig], selectedIndex: Int) {
        ...
        for (index, config) in tabs.enumerated() {
            if config.isSearchTab {
                ...
                if let assetName = config.assetIcon, !assetName.isEmpty {
                    if let asset = UIImage(named: assetName) {
                        image = resizedForTabBar(asset).withRenderingMode(.alwaysTemplate)
                    }
                    if let selectedName = config.assetSelectedIcon, !selectedName.isEmpty {
                        if let selectedAsset = UIImage(named: selectedName) {
                            selectedImage = resizedForTabBar(selectedAsset).withRenderingMode(.alwaysTemplate)
                        }
                    } else {
                        selectedImage = image
                    }
                } else if let symbol = config.sfSymbol {
    ...
    private func handleMethodCall(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "enableNativeTabBar":
            ...
            let tabs = tabsData.compactMap { data -> TabConfig? in
                ...
                let assetIcon = data["assetIcon"] as? String
                let assetSelectedIcon = data["assetSelectedIcon"] as? String
                ...
                return TabConfig(
                    ...
                    sfSymbol: symbol,
                    assetIcon: assetIcon,
                    assetSelectedIcon: assetSelectedIcon,
                    isSearchTab: isSearch,
